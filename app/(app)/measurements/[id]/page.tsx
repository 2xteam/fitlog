...
import { FIELDS, SEGMENT_LABELS, pick, type SegmentKey } from "@/lib/inbody";
...
export default function MeasurementDetailPage() {
  ...
  return (
    <div>
      ...
      <Sheet eyebrow="VALUES" headline="측정 수치">
        <div style={{ marginTop: 16 }}>
          {shown.map((f) => {
            const value = pick(row, f.path);
              ...
              >
                ...
                >
                  ...
                  <span style={{ fontWeight: 700 }}>
                    {value}
                    <span style={{ fontSize: "0.7rem", marginLeft: 3 }}>{f.unit}</span>
                  </span>
                </div>

                {/* 결과지에 인쇄된 표준범위를 막대로 재현 */}
                {ratio != null ? (
      ...
      <Sheet>
        <Link href="/measurements" className="btn btn--ghost btn--block">
          목록으로
        </Link>
        <button
          type="button"
